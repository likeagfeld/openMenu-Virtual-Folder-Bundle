...
draw_dcnow_tr(void) {
    z_set_cond(205.0f);
    bool connect_started_this_frame = false;
    ...
    if (dcnow_is_connecting && dcnow_connect_cooldown_pending) {
        ...
        if (remaining_ms > 0) {
            ...
            dcnow_connect_cooldown_pending = false;
            snprintf(connection_status, sizeof(connection_status), "Starting connection...");
            dcnow_start_connect_worker_or_sync();
            connect_started_this_frame = true;
        }
    }
    ...
    } else if (dcnow_is_connecting && !dcnow_connect_cooldown_pending && !connect_started_this_frame && !dcnow_worker_is_busy()) {
